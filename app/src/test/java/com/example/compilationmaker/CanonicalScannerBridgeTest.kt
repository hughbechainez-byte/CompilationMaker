...
class CanonicalScannerBridgeTest {
    @Test
    fun profileMappingKeepsCanonicalFastAndAccurateChoices() {
        assertEquals(ScanProfile.FAST, canonicalProfileFor(500L))
        assertEquals(ScanProfile.PRECISE, canonicalProfileFor(250L))
        assertEquals(ScanProfile.PRECISE, canonicalProfileFor(3_000L))
        assertEquals(ScanProfile.BALANCED, canonicalProfileFor(10_000L))
        assertEquals(ScanProfile.FAST, canonicalProfileFor(30_000L))
        assertEquals(ScanProfile.FAST, canonicalProfileFor(60_000L))
        assertEquals(ScanProfile.FAST, canonicalProfileFor(180_000L))
        assertEquals(ScanProfile.MONOTONIC_3_MIN, canonicalProfileFor(180_000L, "MONOTONIC_3_MIN"))
        assertEquals(ScanProfile.QUICK_5_MIN, canonicalProfileFor(300_000L, "QUICK_5_MIN"))
    }

    @Test
    fun progressMapsIntoCompilationScanRangeWithoutOvertakingExport() {
        val coarse = mapCanonicalProgress(DetectionProgress("coarse_scan", "Checkpoint", 60))
        val refining = mapCanonicalProgress(DetectionProgress("refining", "Refining", 98))
        val completed = mapCanonicalProgress(DetectionProgress("completed", "Done", 100))
    ...
    fun resultMappingUsesActualPtsAndProducesExactClipPlan() {
        ...
        val mapped = mapCanonicalResult(result)
